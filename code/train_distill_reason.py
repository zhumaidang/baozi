'''
 ...
 # @ Create Time: 2025-05-21 15:57:38
 # @ Modified by: zhu jiangqiang
 # @ Modified time: 2025-05-21 17:10:33
 # @ Description:
 '''
...
def init_distributed_mode():
    ...
    dist.init_procecss_group(backend="nccl")
...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        if step % args.log_interval == 0:
            ...
            Logger(
                'Epoch:[{}/{}] ({}/{}) loss:{:.3f} lr: {:.2f} epoch_time:{}min:'.format(
                    epoch + 1,
                    args.epochs,
                    step,
                    iter_per_epoch,
                    loss.item(),
                    optimizer.param_group[-1]['lr'],
...
if __name__ == "__main__":
    ...
    tokens_per_iter = args.batch_size * lm_config.max_seq_len  # 每次迭代的token数量
    device_type = "cuda" if "cuda" in args.device else "cpu"

    args.wandb_run_name = f"MiniMind-Distill-Reason-Epoch-{args.epochs}-BatchSize-{args.batch_size}-LearningRate-{args.learning_rate}"

    ctx = nullcontext() if device_type == "cpu" else torch.cuda.amp.autocast()  # 上下文管理器  负责在CPU或GPU处理
