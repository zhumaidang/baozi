...
def init_model(args):
    ...
    if args.load == 0:
        moe_path = '_moe' if args.user_moe else ''
        ...
        model = AutoModelForCausalLM.from_pretrained(transformers_model_path, trust_remote_code=True, device_map='auto')
...
def main():
    ...
    message = []
    for idx, prompt in enumerate(prompts if test_mode == 0 else iter(lambda: input('input:'), '')):
        setup_seed(random.randint(0, 2048))
        if test_mode == 0: print(f'input: {prompt}')

        messages = message[-args.history_cnt:] if args.history_cnt else []
