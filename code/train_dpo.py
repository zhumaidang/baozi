...
def dpo_loss(ref_probs, probs, mask, beta):
    # ref_probs 和 probs 的形状都为：(batch_size, seq_len)
    seq_lengths = mask.sum(dim=1, keepdim=True)   # shape: (batch_size, 1)
    ref_probs = (ref_probs * mask).sum(dim=1) / seq_lengths.squeeze()  # shape: (batch_size,)
    ...
    for step, batch in enumerate(train_loader):
        ...
        with ctx:
            with torch.no_grad():
                ...
                ref_logits = ref_outputs.logits  # shape: (batch_size, seq_len, vocab_size)
            ref_probs = logits_to_probs(ref_logits, y)  # shape: (batch_size, seq_len) 表示每个位置对应目标token的对数概率
...
def init_model(lm_config):
    tokenizer = AutoTokenizer.from_pretrained('./model/minimind_tokenzier')
